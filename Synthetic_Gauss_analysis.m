...
MI_start = 1e-4; MI_end = 4; n_MI=100;
...
dir_out = "../Null_model_figures/Different_PID_study/high_syn_Null_rand_S"+...
          sprintf('%d',S)+"T"+sprintf('%d',T)+"/"; %high_syn_Null_lin_
...
%     if(mod(i,10)==0), disp(i); end
    ...
    null_PIDs{i} = MI_null_model_Gauss(MI,[],[],1000,"all");

    n=0;
    norm_syn=0;
    while norm_syn<0.9
        % generate a random Gaussian system
        Sigma_full = Gaussian_from_MI(MI,S,T);
        
        % MMI
        [UnX, UnY, Red, Syn] = PID_MMI_Gaussian(Sigma_full,S,T);
        PIDs(:,i,1) = [UnX, UnY, Red, Syn];
...
%         norm_syn = Syn/MI;
    ...
    % Idep
    [UnX, UnY, Red, Syn] = PID_Idep_Gaussian(Sigma_full,S,T);
    PIDs(:,i,2) = [UnX, UnY, Red, Syn];
    % Iccs 
    [UnX, UnY, Red, Syn] = PID_Iccs_Gaussian(Sigma_full,S,T);
    PIDs(:,i,3) = [UnX, UnY, Red, Syn];
