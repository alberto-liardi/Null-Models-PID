...
for k = 1:length(Sources)
    for i = 1:length(MI_list)
        ...
        for j = 1:n_sample
            ...
            % source covariance
            Sigma_s = wishrnd(eye(S),S);
            % linear coefficients
            A = normrnd(0,1,T,S);
            % target conditional covariance
            Sigma_u = wishrnd(eye(T),T);
...
%% 
    ID = "S" + sprintf('%01d',S) + "T" + sprintf('%01d',T)+"ndof";
    fprintf("done "+ ID+"\n");
